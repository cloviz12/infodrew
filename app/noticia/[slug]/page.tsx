...
const SECTION_COPY = {
  COLOMBIA: { label: "Colombia", tone: "colombia" as const },
  CAUCA: { label: "Cauca", tone: "cauca" as const },
};
...
export default async function PostDetailPage({
  ...
  return (
    <>
      <ArtCover
        slot={{ src: post.coverImage ?? undefined }}
        tone={section.tone}
        className="animated flex h-[42vh] items-end sm:h-[54vh]"
      >
        <div className="w-full px-4 pb-9 pt-24 sm:px-6 sm:pb-12">
          <div className="mx-auto max-w-3xl">
            <div className="reveal-up flex flex-wrap items-center gap-2.5">
              <SectionBadge section={post.section} category={post.category} />
            </div>
            <h1 className="reveal-up delay-1 mt-4 font-display text-3xl font-black leading-[1.1] text-white sm:text-5xl">
              {post.title}
            </h1>
            <p className="reveal-up delay-2 mt-4 text-xs font-semibold uppercase tracking-wide text-white/60">
